...
type FeishuPostTextNode = { tag: "text"; text: string };
type FeishuPostAtNode = { tag: "at"; user_id: string };

function buildFeishuPostContentFromText(input: string): {
  usesPost: boolean;
  contentJson: string;
} {
  // Mention syntax supported by this plugin (outbound):
  //   @{DisplayName|ou_xxx}
  // This converts into a Feishu "post" message with <at> nodes so it triggers notifications.
  const pattern = /@\{([^|{}]+)\|(ou_[a-zA-Z0-9_]+)\}/g;
  let lastIndex = 0;
  const nodes: Array<FeishuPostTextNode | FeishuPostAtNode> = [];
  let match: RegExpExecArray | null;
  let usesPost = false;

  while ((match = pattern.exec(input))) {
    usesPost = true;
    const [full, _name, openId] = match;
    const start = match.index;
    if (start > lastIndex) {
      nodes.push({ tag: "text", text: input.slice(lastIndex, start) });
    }
  ...
  if (!usesPost) {
    return { usesPost: false, contentJson: JSON.stringify({ text: input }) };
  }

  if (lastIndex < input.length) {
    nodes.push({ tag: "text", text: input.slice(lastIndex) });
  }

  // Feishu post schema: content is an array of paragraphs, each paragraph is an array of nodes.
  // We use a single paragraph.
  const post = {
    zh_cn: {
      title: "",
      content: [nodes],
  ...
  return { usesPost: true, contentJson: JSON.stringify(post) };
}
...
export async function sendFeishuTextMessage(params: {
  ...
  const { usesPost, contentJson } = buildFeishuPostContentFromText(params.text);
  const body = {
    msg_type: usesPost ? "post" : "text",
    content: contentJson,
  };

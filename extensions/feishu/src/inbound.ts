...
function extractMentionKeysFromText(text: string): string[] {
  const matches = text.match(/@_user_\d+/gi);
  if (!matches) return [];
...
function normalizeFeishuMention(mention: Record<string, unknown>): FeishuMention {
  const id = isRecord(mention.id) ? mention.id : mention;
  const openId = readString(id.open_id) ?? readString(id.openId);
  const userId = readString(id.user_id) ?? readString(id.userId);
  const unionId = readString(id.union_id) ?? readString(id.unionId);
  const name =
    readString(mention.name) ??
    readString(mention.user_name) ??
    readString(id.name) ??
    readString(id.user_name);
  const key =
    readString(mention.key) ?? readString(mention.mention_key) ?? readString(id.key);
  const isAll = [openId, userId, unionId].some((value) => value === "all");
  return { key, name, openId, userId, unionId, isAll, raw: mention };
}

export function normalizeFeishuMentions(params: {
  mentions: Array<Record<string, unknown>>;
  text: string;
}): FeishuMention[] {
  if (params.mentions.length === 0) return [];
  const keys = extractMentionKeysFromText(params.text);
  return params.mentions.map((mention, index) => {
    const normalized = normalizeFeishuMention(mention);
    const fallbackKey = keys[index];
    return {
      ...normalized,
      key: normalized.key ?? fallbackKey,
    };
  });
}
...
function extractTextFromMessageContent(content: string): {
  text: string;
  mentions: Array<Record<string, unknown>>;
} {
  ...
  if (parsed.ok && isRecord(parsed.value)) {
    ...
    // post (rich text)
    // content example: {"zh_cn": {"content": [[{"tag":"text","text":"hi"},{"tag":"at","user_id":"ou_xxx"}]]}}
    const zh = isRecord(parsed.value.zh_cn) ? parsed.value.zh_cn : null;
    const contentBlocks = zh && Array.isArray(zh.content) ? zh.content : null;
    if (contentBlocks) {
      let out = "";
      let mentionIndex = 0;
      ...
      for (const paragraph of contentBlocks) {
        ...
        for (const node of paragraph) {
          ...
          if (tag === "text") {
            out += readString(node.text) ?? "";
          } else if (tag === "at") {
            const openId = readString(node.user_id) ?? "";
            if (!openId) {
              out += "@";
              continue;
            }
            ...
            out += key;
            mentions.push({ key, id: { open_id: openId } });
          }
        }
        out += "\n";
      }
      out = out.trim();
      if (out) return { text: out, mentions };
    }
  }
...
function wasBotMentioned(params: {
  ...
  return params.mentions.some((mention) => {
    if (botOpenId && mention.openId && botOpenId === mention.openId) return true;
    if (botUserId && mention.userId && botUserId === mention.userId) return true;
    return false;
  });
}
...
export async function processFeishuMessage(params: {
  ...
  if (normalizedType !== "text" && normalizedType !== "post") {
    logVerbose(core, runtime, `skip unsupported message (type=${messageType})`);
    return;
  }
  ...
  const allMentions =
    extracted.mentions.length > 0 ? [...mentions, ...extracted.mentions] : mentions;
  const mentionDetails = normalizeFeishuMentions({ mentions: allMentions, text: rawBody });
  const mentionedUserIds = Array.from(
    new Set(
      mentionDetails
        .flatMap((mention) => [mention.openId, mention.userId, mention.unionId])
  ...
  let mentionGateResult: { effectiveWasMentioned: boolean; shouldSkip: boolean } | null = null;
  ...
  if (isGroup) {
    ...
    if (requireMention || mentionDetails.length > 0) {
      try {
    ...
    mentionGateResult = resolveMentionGatingWithBypass({
      isGroup: true,
      requireMention,
      canDetectMention: true,
      wasMentioned: botMentioned,
      implicitMention: false,
      hasAnyMention: mentionDetails.length > 0,
      allowTextCommands,
      hasControlCommand: core.channel.text.hasControlCommand(rawBody, config),
      commandAuthorized: commandAuthorized === true,
    });
    if (mentionGateResult.shouldSkip) {
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    WasMentioned: isGroup ? mentionGateResult?.effectiveWasMentioned : undefined,
    MentionedUsers: mentionDetails.length > 0 ? mentionDetails : undefined,

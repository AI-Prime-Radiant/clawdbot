...
const warnedMissingEncryptKey = new Set<string>();
...
function extractFeishuMessageFromEnvelope(
  ...
  // content may be empty for some message types; don't drop the event here.
  if (!messageId || !chatId || !chatType || !messageType) return null;
...
async function handleEventPayload(params: {
  ...
  if (!isRecord(params.event)) {
    logVerbose(
      params.core,
      params.runtime,
      `[${params.account.accountId}] ws event ignored (not an object)`,
    );
    return;
  }

  if (readString(params.event.encrypt) && !params.account.config.encryptKey?.trim()) {
    if (!warnedMissingEncryptKey.has(params.account.accountId)) {
      warnedMissingEncryptKey.add(params.account.accountId);
      logVerbose(
        params.core,
        params.runtime,
        `[${params.account.accountId}] ws event payload is encrypted but channels.feishu.encryptKey is not set`,
      );
    }
  }
  ...
  const eventType = readString(
    isRecord(normalized.header) ? normalized.header.event_type : undefined,
  );
  if (eventType && eventType !== "im.message.receive_v1") {
    logVerbose(
      params.core,
      params.runtime,
      `[${params.account.accountId}] ws event ignored (event_type=${eventType})`,
    );
  }
  ...
  if (!extracted) {
    logVerbose(
      params.core,
      params.runtime,
      `[${params.account.accountId}] ws event ignored (unrecognized payload shape)`,
    );
    return;
  }
...
async function monitorSingleSession(params: {
        ...
        try {
          ...
          if (!pingLogged) {
            ...
            logVerbose(
              core,
              options.runtime,
              `[${options.account.accountId}] ws ping started (intervalMs=${pingState.intervalMs})`,
            );
          }
        } catch (err) {
  ...
  const handleControlFrame = (frame: FrameMessage) => {
    debugCounts.control += 1;
    const headerMap = resolveHeaderMap(frame.headers);
    const type = headerMap.type?.trim().toLowerCase();
    if (debugCounts.control <= 3) {
      logVerbose(
        core,
        options.runtime,
        `[${options.account.accountId}] ws control frame type=${type || "unknown"}`,
      );
    }
  ...
  const handleDataFrame = async (frame: FrameMessage) => {
    ...
    if (type !== MESSAGE_TYPE_EVENT) {
      debugCounts.ignored += 1;
      if (debugCounts.ignored <= 3) {
        logVerbose(
          core,
          options.runtime,
          `[${options.account.accountId}] ws data frame ignored (type=${type || "unknown"})`,
        );
      }
      return;
    }
    ...
    if (!messageId || !Number.isFinite(sum) || !Number.isFinite(seq) || sum <= 0 || seq < 0) {
      if (debugCounts.event <= 3) {
        const keys = Object.keys(headerMap).sort().join(",");
        logVerbose(
          core,
          options.runtime,
          `[${options.account.accountId}] ws event frame missing headers (keys=${keys})`,
        );
      }
      return;
    }
    ...
    if (debugCounts.event <= 3) {
      const encoding = frame.payloadEncoding?.trim() || "raw";
      logVerbose(
        core,
        options.runtime,
        `[${options.account.accountId}] ws event payload assembled (chunks=${sum}, encoding=${encoding}, bytes=${merged.length})`,
      );
    }
  ...
  const closePromise = new Promise<void>((resolve) => {
    ...
    const onMessage = (event: { data: unknown }) => {
      void (async () => {
        ...
        if (!bytes) return;
        if (debugCounts.control + debugCounts.data === 0) {
          logVerbose(
            core,
            options.runtime,
            `[${options.account.accountId}] ws first frame received (bytes=${bytes.length})`,
          );
        }
...
export async function monitorFeishuLongConnection(
  ...
  while (!options.abortSignal.aborted) {
    ...
    try {
      if (core.logging.shouldLogVerbose()) {
        try {
          const url = new URL(connectConfig.connectUrl);
          logVerbose(
            core,
            options.runtime,
            `[${options.account.accountId}] ws config ok (host=${url.host}, serviceId=${connectConfig.serviceId}, pingIntervalMs=${connectConfig.pingIntervalMs})`,
          );
        } catch {
          // Ignore URL parse failures; we'll fail in connectWebSocket anyway.
        }
      }
      ...
      logVerbose(core, options.runtime, `[${options.account.accountId}] ws connected`);
      attempts = 0;
